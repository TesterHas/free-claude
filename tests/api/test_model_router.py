...
def settings():
    settings = Settings()
    settings.model = "deepseek/fallback-model"
...
def test_model_router_resolves_default_model(settings):
    ...
    assert resolved.provider_model_ref == "deepseek/fallback-model"
...
def test_model_router_applies_haiku_override(settings):
    settings.model_haiku = "open_router/anthropic/claude-3-haiku"
    ...
    assert routed.request.model == "anthropic/claude-3-haiku"
    assert routed.resolved.provider_model_ref == "open_router/anthropic/claude-3-haiku"


def test_model_router_applies_sonnet_override(settings):
    settings.model_sonnet = "deepseek/deepseek-chat"
    ...
    assert (
        routed.resolved.provider_model_ref == "deepseek/deepseek-chat"
    )
...
def test_model_router_routes_gateway_encoded_provider_model_directly(settings):
    routed = ModelRouter(settings).resolve_messages_request(
        MessagesRequest(
            model="anthropic/deepseek/deepseek-v4-pro",
            max_tokens=100,
            messages=[Message(role="user", content="hello")],
        )
    )
...
def test_model_router_routes_no_thinking_gateway_model_directly(settings):
    ...
    routed = ModelRouter(settings).resolve_messages_request(
        MessagesRequest(
            model="claude-3-freecc-no-thinking/deepseek/deepseek-v4-pro",
            max_tokens=100,
            messages=[Message(role="user", content="hello")],
        )
    )
