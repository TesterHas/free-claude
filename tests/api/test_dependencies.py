...
async def test_get_provider_passes_http_timeouts_from_settings():
    ...
    with (
        ...
        patch("providers.anthropic_messages.httpx.AsyncClient") as mock_http_client,
    ):
        mock_settings.return_value = _make_mock_settings(
...
async def test_get_provider_for_type_different_types():
    ...
    with patch("api.dependencies.get_settings") as mock_settings:
        ...
        assert deepseek is not open_router
...
async def test_cleanup_provider_cleans_all():
    ...
    with patch("api.dependencies.get_settings") as mock_settings:
        ...
        deepseek._client = AsyncMock()
        open_router._client = AsyncMock()

        await cleanup_provider()

        deepseek._client.aclose.assert_called_once()
...
def test_resolve_provider_per_app_uses_separate_registries() -> None:
    ...
    with patch("api.dependencies.get_settings") as mock_settings:
        ...
        p1 = resolve_provider(
            "deepseek", app=cast(Starlette, app1), settings=settings
        )
        p2 = resolve_provider(
            "deepseek", app=cast(Starlette, app2), settings=settings
        )
        assert isinstance(p1, DeepSeekProvider)
