...
@pytest.mark.asyncio
async def test_deepseek_stream_failure_default_logs_exclude_exception_str(caplog):
    ...
    response = FakeResponse(lines=["event: ping", 'data: {"type":"ping"}', ""])

    async def boom(_self, _response):
        raise RuntimeError("SECRET_DEEPSEEK")
        if False:
            yield ""
    ...
    with (
        ...
        _ = [e async for e in provider.stream_response(MockRequest())]

    messages = " | ".join(r.getMessage() for r in caplog.records)
    assert "SECRET_DEEPSEEK" not in messages
    assert "exc_type=RuntimeError" in messages


@pytest.mark.asyncio
async def test_deepseek_stream_failure_respects_verbose_flag(caplog):
    provider = DeepSeekProvider(
        ProviderConfig(
            api_key="k",
            base_url="https://api.deepseek.com/anthropic",
    ...
    messages = " | ".join(r.getMessage() for r in caplog.records)
    assert "SECRET_DEEPSEEK" in messages
