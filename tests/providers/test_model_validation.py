...
def _settings(
    *,
    model: str = "deepseek/deepseek-model",
...
async def test_registry_validation_succeeds_for_all_configured_models() -> None:
    registry = ProviderRegistry(
        {
            "deepseek": FakeProvider(frozenset({"deepseek-model"})),
    ...
    assert registry.cached_model_ids() == {
        "deepseek": frozenset({"deepseek-model"}),
...
async def test_registry_validation_reports_missing_model_with_sources() -> None:
    registry = ProviderRegistry(
        {"deepseek": FakeProvider(frozenset({"different-model"}))}
    )
    settings = _settings(model_sonnet="deepseek/deepseek-model")
    ...
    assert "model=deepseek-model" in message
...
async def test_registry_validation_aggregates_multiple_failures() -> None:
    ...
    message = exc_info.value.message
    assert "sources=MODEL provider=deepseek model=deepseek-model" in message
    assert "problem=missing model" in message
...
async def test_registry_refresh_model_list_cache_keeps_prior_cache_on_failure() -> None:
    registry = ProviderRegistry(
        {"deepseek": FakeProvider(error=RuntimeError("upstream down"))}
    )
    registry.cache_model_ids("deepseek", {"cached-model"})
    settings = _settings(
        model="deepseek/cached-model",
        deepseek_api_key="ds-key",
    )

    await registry.refresh_model_list_cache(settings)

    assert registry.cached_model_ids() == {"deepseek": frozenset({"cached-model"})}
