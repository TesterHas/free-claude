...
class TestProviderRateLimiter:
    ...
    async def test_scoped_instances_are_isolated(self):
        ...
        GlobalRateLimiter.reset_instance()
        deepseek = GlobalRateLimiter.get_scoped_instance(
            "deepseek", rate_limit=10, rate_window=60
        )
        openrouter = GlobalRateLimiter.get_scoped_instance(
            "open_router", rate_limit=20, rate_window=30
        )

        assert deepseek is not openrouter
        deepseek.set_blocked(1.0)

        assert deepseek.is_blocked() is True
