...
This proxy works with any project that uses the Anthropic SDK — not just Claude Code. Point `ANTHROPIC_BASE_URL` at the proxy and it will transparently route requests to DeepSeek (or whichever provider you configure).
...
**3. In your other project, set these environment variables:**
...
No code changes are needed — just the environment variables.

### Switching Between DeepSeek and Real Claude

To use the proxy, set `ANTHROPIC_BASE_URL`:

```bash
# PowerShell
$env:ANTHROPIC_BASE_URL="http://localhost:8082"; $env:ANTHROPIC_API_KEY="freecc"; python your_script.py

# Bash
ANTHROPIC_BASE_URL=http://localhost:8082 ANTHROPIC_API_KEY=freecc python your_script.py
```

To switch back to the real Anthropic API, unset `ANTHROPIC_BASE_URL` (and use your real `ANTHROPIC_API_KEY`). The SDK falls back to Anthropic's servers automatically when `ANTHROPIC_BASE_URL` is not set.

## License

MIT License. © 2026 Nate Herk, Ali Khokhar. See [LICENSE](LICENSE) for details.
